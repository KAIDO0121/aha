...
async def create_user(user: UserCreate, request: Request, db: Session = Depends(get_db)):
    ...
    token = auth_handler.encode_token(user.name, user.email)
...
def dashboard(request: Request, credentials: HTTPAuthorizationCredentials = Security(security)):
    ...
    if not verified:
        return HTMLResponse(f'<p>This the dashboard</p><a href="/api/resend_verification_email">Resend Email Verification</a>')
